...
```php
$orderProcessingMarketplaceClient = new \Yandex\Marketplace\Partner\Clients\OrderProcessingBeruClient();
```
...
Метод `updateOrdersStatuses` изменяет статусы нескольких заказов.

```php
// Изменяем статусы заказов
$response = $orderProcessingClient->updateOrdersStatuses($campaignId,
    ///список заказов
    ["orders" => [
        [
...
```
Подробнее см. в [документации API](https://yandex.ru/dev/market/partner-marketplace-cd/doc/dg/reference/post-campaigns-id-orders-status-update-docpage/).

#### Ярлыки‑наклейки на все грузовые места в заказе

Метод `getDeliveryLabels` формирует ярлыки‑наклейки на все грузовые места в заказе и возвращает ярлыки в PDF‑файле.

```php
// Получаем PDF‑файл
$pdf = $orderProcessingClient->getDeliveryLabels($campaignId, 
    // Идентификатор заказа, обязательный параметр    
    $orderid
);
```
Подробнее см. в [документации API](https://yandex.ru/dev/market/partner-marketplace-cd/doc/dg/reference/get-campaigns-id-orders-id-delivery-labels-docpage/).

#### Ярлык‑наклейка на отдельное грузовое место в заказе

Метод `getDeliveryLabelForBoxes` формирует ярлык‑наклейку на отдельное грузовое место в заказе и возвращает ярлык в PDF‑файле.

```php
// Получаем PDF‑файл
$pdf = $orderProcessingClient->getDeliveryLabelForBoxes($campaignId, 
    // Идентификатор заказа, обязательный параметр    
    $orderid,
    //Идентификатор посылки, обязательный параметр
    $shipmentId,
    //Идентификатор грузового места, обязательный параметр
    $boxId
);
```
Подробнее см. в [документации API](https://yandex.ru/dev/market/partner-marketplace-cd/doc/dg/reference/get-campaigns-id-orders-id-delivery-shipments-id-boxes-id-label-docpage/).

#### Информация на ярлыках‑наклейках

Метод `getDeliveryLabelsData` возвращает информацию на ярлыках, которые клеятся на грузовые места в заказе.
                             
```php
// Получаем информацию на ярлыках
$response = $orderProcessingClient->getDeliveryLabelsData($campaignId, 
     // Идентификатор заказа, обязательный параметр    
     $orderid
);
//Получаем информацию на ярлыках
$parcelBoxLabels = $response->getResult()->getParcelBoxLabels();
```
Подробнее см. в [документации API](https://yandex.ru/dev/market/partner-marketplace-cd/doc/dg/reference/get-campaigns-id-orders-id-delivery-labels-data-docpage/#get-campaigns-id-orders-id-delivery-labels-data).
