...
$recommendedPrices = $priceClient->getRecommendedPrices($campaignId, [
    'offers' => [
        [
            // SKU на Яндексе, обязательный параметр        
...
$recommendedRelationships = $relationshipClient->getRecommendedRelationship($campaignId, [
    'offers' => [
        [
...
$activeRelationships = $activeRelationshipObject->getOfferMappingEntries();
...
$shipmentItemsObject = $shipmentsClient->getShipmentItems($campaignId, $requestId);
